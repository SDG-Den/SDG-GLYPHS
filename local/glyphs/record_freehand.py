"""
Freehand recording overlay.

Divides the screen into a grid of cells.  The user draws the same glyph
freehand in each cell.  When all cells are filled a dialog prompts for
the glyph name and output string, then persists it.
"""
...
class FreehandRecordOverlay:
    """Fullscreen grid overlay for recording new glyphs with freehand
    (unsnapped) strokes.

    The screen is divided into *N* cells (default 4).  The user draws the
    same glyph shape in each cell.  When all cells are filled, a dialog
    prompts for the glyph name and output string.
    """
    ...
    def _on_button_release(self, widget, event):
        ...
        self._grid[self.current_cell].append(list(stroke))
    ...
    def _prompt_mapping(self):
        ...
        if resp == Gtk.ResponseType.OK and name and string:
            templates = []
            for cell_strokes in self._grid:
                if cell_strokes:
                    s = cell_strokes[0]
                    alloc = (self.da.get_allocated_width(),
                             self.da.get_allocated_height())
                    eps = max(alloc) * 0.008
                    simplified = rdp_simplify(s, eps)
    ...
    def _on_draw(self, widget, cr):
        ...
        for i in range(self.sections):
            ...
            if self._filled[i]:
                cr.set_source_rgba(0.3, 0.8, 0.3, 0.3)
            ...
            for stroke in self._grid[i]:
                self._draw_polyline(cr, stroke, ink_w,
                                    0.7, 0.9, 0.7, 0.9)
